...
public interface DailyWorkDao {
    ...
    @Insert("insert into t_daily_work(flag,create_time,create_userid,update_time,update_userid," +
            "order_track_id,schedule_id,equipment_id,equipment_seq,start_time,end_time,working_count,qualified_count," +
            "unqualified_cout,complete_flag,factory_id,bak) " +
            "values(#{flag},#{createTime},#{createUserid},#{updateTime},#{updateUserid},#{orderTrackId},#{scheduleId}," +
    ...
    @Delete({"delete from t_daily_work where id = #{id}"})
    ...
    // 根据跟踪ID获取生产调度信息
    @Select({"select * from t_daily_work where order_track_id = #{id}"})
    @Results({
            @Result(column="id", property="id", id=true),
            @Result(column="flag", property="flag"),
            @Result(column="create_time", property="createTime"),
    ...
    TDailyWork selectByOrderTrackId(Integer id);
